...
export default function TenantClosureManager() {
    const [blocks, setBlocks] = useState<ClosureBlock[]>([]);
    ...
    const loadBlocks = async () => {
        try {
            const res = await fetch(`/api/tenant/time-off?from=${new Date().toISOString()}`);
            if (res.ok) {
    ...
    const handleAdd = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch(`/api/tenant/time-off`, {
                method: 'POST',
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            const res = await fetch(`/api/tenant/time-off?id=${id}`, {
                method: 'DELETE'
            });
    ...
    return (
        <Card className="mt-8">
            <CardHeader className="flex flex-row items-center justify-between">
                <div>
                    <CardTitle>Business Closures & Holidays</CardTitle>
                    <CardDescription>Configure days when the entire business is closed (e.g., Public Holidays).</CardDescription>
                </div>
                <Dialog open={open} onOpenChange={setOpen}>
                    ...
                    <DialogContent>
                        <DialogHeader>
                            <DialogTitle>Add Business Closure</DialogTitle>
                            <DialogDescription>
                                Set a period where the entire business will be closed.
                            </DialogDescription>
                        </DialogHeader>
